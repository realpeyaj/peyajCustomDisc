...
object FabricPackUpdater {
    ...
    fun register() {
        ServerPlayConnectionEvents.JOIN.register { handler, _, server ->
            ...
            server.execute {
                ...
                if (hash.isNotEmpty()) {
                    try {
                        val promptText = Text.literal(config.prompt)
                        val packet = net.minecraft.network.packet.s2c.common.ResourcePackSendS2CPacket(
                            JAVA_PACK_UUID,
                            zipUrl,
                            hash,
                            config.required,
            ...
            if (geyserDir.exists()) {
                try {
                    ...
                    if (forceReload && PeyajCustomDiscFabric.config.autoReloadGeyser) {
                        ...
                        server?.commandManager?.executeWithPrefix(server.commandSource, "geyser reload")
                    }
    ...
    private fun isBedrockPlayer(player: ServerPlayerEntity): Boolean {
        try {
